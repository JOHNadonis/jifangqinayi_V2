...
export class SyncController {
  ...
  async pushChanges(@Body() actions: SyncAction[], @Request() req: any) {
    ...
    return this.syncService.syncFromClient(actionsWithClient, req.project.id);
  }
  ...
  async pullChanges(@Query('lastSyncTime') lastSyncTime: string, @Request() req: any) {
    return this.syncService.getUpdatesForClient(req.user.sub, parseInt(lastSyncTime, 10), req.project.id);
  }
}
